...
- **`rp2040_link.h`** — raw IDF UART2 driver for the ESP32↔RP2040 link (TX 19 / RX 20; fully detaches the S3's USB PHY from those pads, RTC-domain override included). `playAlarm(volume)` sends the volume command (0x11–0x15). Includes permanent link diagnostics: pad-level read-back (`tx_pad/rx_pad`), a raw RX hex monitor, and a 30 s round-trip ping.
